...
class DispatcherController extends Controller
{
    ...
    {
        $Type = $request->type;
        $Trips = UserRequests::with('user', 'provider','service_type', 'filter')
                    ->orderBy('id','desc');

        if($Type == "SCHEDULED"){
            $Trips = $Trips->where('status',$Type);
        }else if($Type == "ONGOING"){
            $Trips = $Trips->where('status', '<>', 'SEARCHING')
        ...
        return view('admin.dispatch.index', compact('Trips', 'Type'));
    }
    ...
    public function getAssign($id)
    {
        ...
        $ActiveProviders = ProviderService::AvailableServiceProvider($UserRequest->service_type_id)
                ->get()
                ->pluck('provider_id');
        ...
        $Providers = Provider::with('service.service_type')->whereIn('id', $ActiveProviders)
            ->where('status', 'approved')
            ->whereRaw("(1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance")
            ->get();

        return view('admin.dispatch.assign', compact('Providers', 'UserRequest'));
    }
    ...
    public function assign($request_id, $provider_id)
    {
        try {
            ...
            if(Auth::guard('admin')->user()){
                return redirect()->route('admin.dispatcher.trips')->with('flash_error', 'Something Went Wrong!');
            }elseif(Auth::guard('dispatcher')->user()){
    ...
    public function store(Request $request) {
        ...
        if($ActiveRequests > 0) {
            return back()->with('flash_error', 'Already request is in progress for this User. Try again later');
        }
        ...
        if($request->has('schedule_time')){
            try {
                ...
                return back()->with('flash_error', 'Already request is Scheduled on this time for this User.');
        ...
        try{
            ...
            $UserRequest->d_latitude = $request->d_latitude ? : 0;
            $UserRequest->d_longitude = $request->d_longitude ? : 0;
            // $UserRequest->route_key = $route_key;

            $UserRequest->distance = $request->distance ? : 0;
            ...
            if($request->has('provider_auto_assign')) {
                ...
                return redirect()->route('admin.dispatcher.trips');
            } else {
                return redirect()->route('admin.dispatcher.getassign', ['id' => $UserRequest->id]);
            }
