    ...
    _requestPoll(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                // Initialize Chat Socket / Update it 

                // console.log('Ajax Response', data);
                if(data.requests.length > 0) {
                    // console.log(data.requests);
                    // console.log('data.requests[0]', data.requests[0].request);
                    this.setState({request: data.requests[0].request,service_type:data.requests[0].service_type});
                ...
                <div className="modal-dialog" role="document">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <div className="incoming-img bg-img" id="user-image" style={{ backgroundImage: 'url(/storage/' + this.props.request.user.picture + ')'}}></div>
                            <div className="text-center">
                                <h3 id="usser-name">{this.props.request.user.first_name} {this.props.request.user.last_name}</h3>
                                <h4><b>Description</b> : {this.props.request.description}</h4>
                                <h4><b>Address</b> : {this.props.request.s_address}</h4>
                                <h4><b>Service Type</b> : {this.props.request.service_type.name}</h4>
                            </div>
...
class TripArrivedButton extends React.Component {
    render() {
        return (
            <div>
                <form id="ARRIVED_FORM" encType="multipart/form-data">
...
class TripPickedButton extends React.Component {
    render() {
        return (
            <div>
                <form id="PICKEDUP_FORM" encType="multipart/form-data" action={"/provider/request/"+this.props.request}  method="POST">
                    <label>Choose Before Image</label>
                    <input type="file" name="before_image" className="form-control" id="before_image"/>
                    <br/>
                    <input type="text" name="before_comment" className="form-control" placeholder="Comments" />
                    <button type="submit" className="full-primary-btn fare-btn">Start Service</button>
        ...
        return (
            <div>
                 <form id="DROPPED_FORM" encType="multipart/form-data" action={"/provider/request/"+this.props.request} method="POST">
                    <label>Choose After Image</label>
                    <input type="file" name="after_image" className="form-control" id="after_image"/>
                     <br/>
                    <input type="text" name="after_comment" className="form-control" placeholder="Comments"/>
                    <button type="submit" className="full-primary-btn fare-btn">End Service</button>
...
class TripCompletedButton extends React.Component {
    render() {
        return (
            <div>
                <form id="COMPLETED_FORM" encType="multipart/form-data">
...
class TripDetails extends React.Component {
    ...
    render() {
        // console.log('Trip Details Component');
...
class TripComponent extends React.Component {
    ...
    getButtons() {
        switch(this.props.request.status) {
            ...
            case "ARRIVED": 
                return (
                    <TripPickedButton submit={this._submit.bind(this)} request={this.props.request.id} />
                );
                break;
            case "PICKEDUP": 
                return (
                    <TripDroppedButton submit={this._submit.bind(this)} request={this.props.request.id} />
                );
                break;
            case "DROPPED": 
                return (
                    <TripCompletedButton submit={this._submit.bind(this)} request={this.props.request.id} />
                );
                break;
            case "COMPLETED": 
                return (
